...
public class GoogleCalendarEventsConnector implements EventsConnector {
   ...
   */
  private static Credential getCredentials(String googleClientSecret2, final NetHttpTransport HTTP_TRANSPORT) throws IOException {
    // Load client secrets.
  ...
  public static void main(String... args) throws IOException, GeneralSecurityException {
    new GoogleCalendarEventsConnector("bau").readEvents();
  }

  private String googleClientSecret;

  public GoogleCalendarEventsConnector(String googleClientSecret) {
  ...
  public io.vavr.collection.List<io.vavr.collection.List<String>> readEvents() {
    try {
      ...
      Calendar service = new Calendar.Builder(HTTP_TRANSPORT, JSON_FACTORY, getCredentials(googleClientSecret, HTTP_TRANSPORT))
          .setApplicationName(APPLICATION_NAME).build();

      // List the next 10 events from the primary calendar.
      DateTime now = new DateTime(System.currentTimeMillis());
      Events events = service.events().list("ro.romanian#holiday@group.v.calendar.google.com").setMaxResults(10).setTimeMin(now)
